...
import { useGetInvoiceByIdQuery, useUpdateInvoiceMutation } from './invoiceApiSlice';


const EditInvoice = () => {
 
 const {id} = useParams();   
 const [createNewInvoice] = useUpdateInvoiceMutation();   
 const {data: invoice} = useGetInvoiceByIdQuery(id);
 ...
 const onSubmit = async (data) =>{
    ...
    await createNewInvoice( {...data, id}).unwrap();
    reset({
        name:'',
        email:'',
        phone:'',
    ...
    <div className='flex max-w-2xl shadow border-b mx-auto mt-24 mb-96'>
      <div className='px-8 py-8'>
        <div className='font-thin tracking-wider text-2xl'>
            <h1 className='text-center mb-4'>Update an Invoice</h1>
        </div>
